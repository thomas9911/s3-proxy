...
pub fn internal_path(config: &Config, namespace: &str, bucket: &str, suffix: &str) -> Option<String> {
    match config.storage_layout {
        StorageLayout::Namespaced => Some(format!("{namespace}/.s3-proxy/{suffix}")),
        StorageLayout::SingleBucket => bucket_prefix(config, namespace, bucket)
            .map(|_| format!(".s3-proxy/{suffix}")),
    }
}
